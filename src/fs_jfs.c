...
int jfs_mkfs(cdico *d, char *partition, char *fsoptions, char *mkfslabel, char *mkfsuuid)
{
    ...
    if (exec_command(command, sizeof(command), NULL, NULL, 0, NULL, 0, "jfs_mkfs -V")!=0)
    {   errprintf("jfs_mkfs not found. please install jfsutils on your system or check the PATH.\n");
        return -1;
    }
