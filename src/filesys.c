...
u64 check_prog_version(char *prog)
{
	...
	// check that mount.ntfs-3g is available and that it supports "streams_interface"
	if (exec_command(command, sizeof(command), NULL, 0, NULL, 0, "which %s", prog)!=0)
	{	errprintf("%s not found.\n", prog);
		return -1;
	}
	
	memset(stderrbuf, 0, sizeof(stderrbuf));
	exec_command(command, sizeof(command), NULL, 0, stderrbuf, sizeof(stderrbuf), "%s -V", prog);
